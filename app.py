...
def render_home_page():
    ...
    st.markdown('<h1 class="main-header">ðŸŽ¨ SD-DarkMaster-Pro</h1>', unsafe_allow_html=True)
    ...
    with col1:
        status = "âœ… Ready" if st.session_state.setup_complete else "âŒ Pending"
    ...
    with col4:
        webui_status = "ðŸŸ¢ Running" if st.session_state.webui_process else "âš« Stopped"
        st.metric("WebUI Status", webui_status)
    
    # Platform Information
    st.subheader("ðŸ–¥ï¸ Platform Information")
    ...
    with col1:
        if st.button("âš™ï¸ Setup Environment", use_container_width=True, type="primary"):
            st.switch_page("Setup")
    
    with col2:
        if st.button("ðŸ“¦ Browse Models", use_container_width=True):
            st.switch_page("Models")
    
    with col3:
        if st.button("ðŸ’¾ Manage Downloads", use_container_width=True):
    ...
    if st.session_state.activity_logs:
        with st.container():
            ...
            for log in st.session_state.activity_logs[-10:]:
                icon = "âœ…" if log['level'] == "success" else "â„¹ï¸" if log['level'] == "info" else "âš ï¸" if log['level'] == "warning" else "âŒ"
...
def render_setup_page():
    ...
    # Environment Status
    st.subheader("ðŸ–¥ï¸ Environment Status")
    platform = st.session_state.platform_info
    ...
    # Setup Actions
    st.subheader("ðŸ”§ Setup Actions")
    
    col1, col2, col3 = st.columns(3)
    
    with col1:
        if st.button("ðŸ“ Create Directories", use_container_width=True):
            ...
            status_text.text("âœ… All directories created!")
            add_activity_log("Created directory structure", "success")
            st.session_state.setup_complete = True
            st.rerun()
    
    with col2:
        if st.button("ðŸ“¦ Install Dependencies", use_container_width=True):
            with st.spinner("Installing dependencies..."):
                ...
                st.success("âœ… Dependencies installation completed!")
    
    with col3:
        if st.button("ðŸ§ª Run System Test", use_container_width=True):
            with st.spinner("Running system tests..."):
                # Test GPU
                returncode, output = run_command("nvidia-smi")
                gpu_test = "âœ… GPU Available" if returncode == 0 else "âŒ No GPU"
                ...
                for pkg in packages:
                    try:
                        __import__(pkg)
                        package_tests.append(f"âœ… {pkg}")
                    except ImportError:
                        package_tests.append(f"âŒ {pkg}")
...
def render_models_page():
    ...
    st.title("ðŸ“¦ Model Selection & Management")
    ...
    if total_selected > 0:
        ...
        col1, col2, col3 = st.columns(3)
        with col1:
            if st.button("ðŸ’¾ Save Selections", type="primary", use_container_width=True):
                save_selections()
        with col2:
            if st.button("ðŸ“¥ Add to Download Queue", use_container_width=True):
                add_selections_to_queue()
        with col3:
            if st.button("ðŸ—‘ï¸ Clear All Selections", use_container_width=True):
            ...
            with st.container():
                ...
                if isinstance(model_info, dict):
                    ...
                    if model_info.get('inpainting'):
                        st.caption("ðŸŽ¨ Inpainting model")
                
                # Selection button
                button_text = "âœ… Selected" if is_selected else "âž• Select"
...
def render_civitai_search():
    ...
    st.subheader("ðŸ” CivitAI Model Search")
    ...
    if 'civitai_results' in st.session_state and st.session_state.civitai_results:
        ...
        for idx, model in enumerate(st.session_state.civitai_results[:9]):  # Show first 9
            ...
            with col2:
                ...
                st.caption(f"Rating: {'â­' * int(model.get('stats', {}).get('rating', 0))}")
            
            with col3:
                if st.button("ðŸ“¥ Add to Queue", key=f"civitai_{idx}", use_container_width=True):
...
def add_selections_to_queue():
    ...
    if added_count > 0:
        st.success(f"âœ… Added {added_count} models to download queue!")
...
def render_downloads_page():
    ...
    st.title("ðŸ’¾ Download Management")
    
    # Download queue overview
    st.subheader("ðŸ“‹ Download Queue")
    ...
    if st.session_state.download_queue:
        ...
        for idx, item in enumerate(st.session_state.download_queue):
            ...
            with col4:
                if st.button("ðŸ—‘ï¸", key=f"remove_{idx}", help="Remove from queue"):
                    st.session_state.download_queue.pop(idx)
                    st.rerun()
        
        # Download controls
        st.subheader("â¬‡ï¸ Download Controls")
        col1, col2, col3 = st.columns(3)
        
        with col1:
            if st.button("ðŸš€ Start Downloads", type="primary", use_container_width=True):
                start_downloads()
        ...
        st.info("ðŸ“¥ Download queue is empty. Add models from the Models page.")
        
        if st.button("ðŸ“¦ Go to Models Page", type="primary"):
            st.switch_page("Models")
    
    # Download history
    st.subheader("ðŸ“š Download History")
...
def start_downloads():
    ...
    status_text.text("âœ… All downloads completed!")
    ...
    if history_dir.exists():
        ...
        if downloaded_files:
            ...
            for file_path in downloaded_files[:10]:  # Show last 10
                ...
                with col3:
                    if st.button("ðŸ—‘ï¸", key=f"del_{file_path.name}", help="Delete file"):
                        file_path.unlink()
                        st.rerun()
        else:
...
def render_launch_page():
    """WebUI launcher page"""
    st.title("ðŸš€ WebUI Launcher")
    
    # WebUI selection
    webui_options = {
        "Forge": {
            "description": "ðŸ”¥ Recommended - Optimized SD WebUI with better performance",
            "repo": "https://github.com/lllyasviel/stable-diffusion-webui-forge.git",
            "compatibility": "95% extensions compatible"
        },
        "Automatic1111": {
            "description": "âš¡ Original SD WebUI - Maximum compatibility",
            "repo": "https://github.com/AUTOMATIC1111/stable-diffusion-webui.git", 
            "compatibility": "100% extensions compatible"
        },
        "ComfyUI": {
            "description": "ðŸŽ¯ Node-based interface - Advanced workflows",
    ...
    st.subheader("ðŸŽ›ï¸ Select WebUI")
    ...
    with col2:
        ...
        if webui_path.exists():
            st.success(f"âœ… {selected_webui} is installed")
        else:
            st.warning(f"ðŸ“¦ {selected_webui} will be installed on first launch")
    
    # Launch configuration
    st.subheader("âš™ï¸ Launch Configuration")
    ...
    # Launch button
    st.subheader("ðŸš€ Launch")
    
    if st.button("ðŸš€ Launch WebUI", type="primary", use_container_width=True):
...
def launch_webui(webui_name: str, webui_info: Dict, port: int, share: bool, xformers: bool, launch_args: str):
    ...
    try:
        ...
        status_placeholder.info("ðŸ“ Preparing workspace...")
        progress_bar.progress(0.1)
        webui_path.parent.mkdir(exist_ok=True)
        
        # Step 2: Clone if needed
        if not webui_path.exists():
            status_placeholder.info(f"ðŸ“¥ Cloning {webui_name} repository...")
            ...
            if returncode != 0:
                st.error(f"âŒ Failed to clone repository: {output}")
                return
            
            add_activity_log(f"Cloned {webui_name} repository", "success")
        
        # Step 3: Prepare launch command
        status_placeholder.info("ðŸ”§ Preparing launch command...")
        ...
        # Step 4: Launch
        status_placeholder.info(f"ðŸš€ Starting {webui_name}...")
        ...
        status_placeholder.info("â³ Starting WebUI (this may take a few minutes)...")
        ...
        while time.time() - start_time < 180:  # 3 minute timeout
            if process.poll() is not None:
                st.error("âŒ WebUI process terminated unexpectedly")
                break
        ...
        # Show result
        if webui_url:
            status_placeholder.success(f"âœ… {webui_name} is running!")
            st.balloons()
            
            col1, col2 = st.columns(2)
            with col1:
                st.success(f"ðŸŒ **WebUI URL:** {webui_url}")
            with col2:
                st.info(f"ðŸ–¥ï¸ **Local URL:** http://localhost:{port}")
            
            add_activity_log(f"Successfully launched {webui_name} at {webui_url}", "success")
            
            # Provide direct link
            st.markdown(f"### ðŸ”— Direct Access")
            st.markdown(f"[**ðŸš€ Open {webui_name} WebUI**]({webui_url})")
            
        else:
            status_placeholder.warning("âš ï¸ WebUI started but URL not detected. Check the logs above.")
            add_activity_log(f"Started {webui_name} but URL not detected", "warning")
    
    except Exception as e:
        st.error(f"âŒ Error launching WebUI: {str(e)}")
        add_activity_log(f"Failed to launch {webui_name}: {str(e)}", "error")

def render_storage_page():
    """Storage management page"""
    st.title("ðŸ§¹ Storage Management")
    
    # Storage overview
    st.subheader("ðŸ“Š Storage Overview")
    ...
    # Storage breakdown
    st.subheader("ðŸ“ Storage Breakdown")
            ...
            if files:
                ...
                for file_path in files[:20]:  # Show first 20
                    if file_path.is_file():
                        ...
                        with col4:
                            if st.button("ðŸ—‘ï¸", key=f"del_{file_path.name[:10]}", help="Delete file"):
                                file_path.unlink()
    ...
    # Cleanup actions
    st.subheader("ðŸ§¹ Cleanup Actions")
    
    col1, col2, col3 = st.columns(3)
    
    with col1:
        if st.button("ðŸ—‘ï¸ Clear Temporary Files", use_container_width=True):
            cleanup_temp_files()
    
    with col2:
        if st.button("ðŸ“Š Refresh Storage Stats", use_container_width=True):
    ...
    with col3:
        with st.expander("âš ï¸ Danger Zone"):
            ...
            if st.button("ðŸ—‘ï¸ Clear All Models"):
                if st.checkbox("I understand this will delete all models"):
                    clear_all_models()
            
            if st.button("ðŸ’£ Reset All Storage"):
...
def reset_all_storage():
    ...
    st.success("âœ… Storage completely reset!")
...
def render_monitor_page():
    ...
    with col4:
        ...
        st.metric("Network", "Active", f"â†“{format_bytes(network.bytes_recv)} â†‘{format_bytes(network.bytes_sent)}")
    
    # Process monitoring
    st.subheader("ðŸ”„ Active Processes")
    
    if st.session_state.webui_process:
        st.success("ðŸŸ¢ WebUI Process Running")
        ...
        try:
            ...
            st.warning("âš ï¸ WebUI process information unavailable")
            st.session_state.webui_process = None
    else:
        st.info("âš« No WebUI process running")
    
    # Live logs
    st.subheader("ðŸ“œ Live Activity Log")
...
def main():
    ...
    with st.sidebar:
        ...
        page = st.radio(
            "Choose a page:",
            ["ðŸ  Home", "âš™ï¸ Setup", "ðŸ“¦ Models", "ðŸ’¾ Downloads", "ðŸš€ Launch", "ðŸ§¹ Storage", "ðŸ“Š Monitor"],
            index=0
        )
    ...
    if page == "ðŸ  Home":
        ...
        render_models_page()
    elif page == "ðŸ’¾ Downloads":
        render_downloads_page()
    elif page == "ðŸš€ Launch":
        render_launch_page()
    elif page == "ðŸ§¹ Storage":
        render_storage_page()
    elif page == "ðŸ“Š Monitor":
        render_monitor_page()
