...
def main():
    ...
    if 'civitai_api' not in st.session_state:
        st.session_state.civitai_api = None
    ...
    api_key = st.text_input(
        ...
        type="password",
        help="Get your API key from https://civitai.com/user/account"
    )
    
    if st.button("🔌 Initialize API"):
        st.session_state.civitai_api = CivitAIBasic(api_key)
        st.success("✅ CivitAI API initialized!")
    
    if st.session_state.civitai_api is None:
        st.warning("⚠️ Please initialize the API first")
        return
